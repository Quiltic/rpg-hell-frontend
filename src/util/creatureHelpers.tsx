...
function changeItemInArray(
    changingArray: Array<any>,
    location: number,
    itemToChangeWith: any
) {
    changingArray[location] = itemToChangeWith;
    return [...changingArray];
}

function dictionaryItems(_items: Item[]) {
    let itemsDict = Object.fromEntries(
        _items.map(({ name, ...rest }) => [name, { name, ...rest }])
    ); // provided by https://stackoverflow.com/questions/61379389/mapping-an-array-of-objects-to-dictionary-in-typescript

    if (Object.keys(itemsDict).includes("Error")) {
        // a catch incase there is an error (error items break everything)
        return {};
    }

    for (const [key, value] of Object.entries(itemsDict)) {
        let itemTags = Object.fromEntries(
            value.tags.map((tag) => {
                ...
                else output = [segments[0], parseInt(segments[1])];

                if (Number.isNaN(output[1])) output[1] = 0;
...
function upgradeItem(itemsDict, _stackEffects: string[]) {
    ...
    for (const effect of _stackEffects) {
        ...
        let effectTags = Object.fromEntries(
            effect
                .substring(effect.indexOf(" - ") + 3)
                .split(",")
                .map((tag) => {
                    let segments = tag.split(" ");
                    let output = [];
...
function createItemLines(updatedItems) {
    ...
    for (const [name, value] of Object.entries(updatedItems)) {
        ...
        value.tags = Object.entries(value.tags).map(
            ([tagName, tagValue]) => `${tagName} ${tagValue}`
        );

        if (
            value.tags.join("").includes("weapon") ||
            value.tags.join("").includes("grenade")
        ) {
            active.push(
                `${capitalizeWords(name)} (Weapon) - ## - ${capitalizeWords(value.tags.join(", ").replace(/ 0/gi, "").replace(", weapon", ""))}\n${value.effect}\n`.replace(
                    "\n\n",
                    "\n"
                )
            );
            ...
            active.push(
                `${capitalizeWords(name)} - # - ${value.tags.join(", ").replace(/ 0/gi, "")}\n${value.effect}\n`.replace(
                    "\n\n",
                    "\n"
                )
            );
            ...
            passive.push(
                `${capitalizeWords(name)} - ${value.effect}\n`.replace(
                    "\n\n",
                    "\n"
                )
            );
...
function createTraitLines(
    ...
    for (const t of _traits) {
        if (t.dice != 0) {
            activeLines.push(
                `${capitalizeWords(t.name)} - ${"#".repeat(t.dice ?? 1) ?? "P"}\n${t.effect}\n`
            );
        } else {
