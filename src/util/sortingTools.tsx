...
function sortArrayByLevel(_list: any[]) {
    ...
    const listSortedByName = _list?.sort(
        (l1: { name: any }, l2: { name: any }) => {
            return (l1.name ?? "") < (l2.name ?? "") ? -1 : 1;
        }
...
function sortSpells(_list: any[]) {
    ...
    listSortedByName = listSortedByName.sort(
        (l1: { stat: any }, l2: { stat: any }) => {
            // get the length of the items req removing OOC because it shouldent be there for this case
...
function sortItems(_list: any[]) {
    ...
    return listSortedByName.sort(
        (
            l1: { rarity: string; tags: string },
            l2: { rarity: string; tags: string }
        ) => {
            const order = [
                "mundane",
                "common",
            ...
            let firstItemLen = getMinOrderIndex(
                l1.tags.split(", "),
                IterativeItemLevels
            );
            let secondItemLen = getMinOrderIndex(
                l2.tags.split(", "),
                IterativeItemLevels
            );
