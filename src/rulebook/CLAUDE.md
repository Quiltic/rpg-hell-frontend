...
A source with `toBlock` (definitions) renders a directive that matches exactly one record as a `contentBlock` mdast node, which `mdast-util-to-hast` turns into a `<div id="definition-…">` holding the parsed markdown blocks; several matches fall back to the list path. `placements.test.ts` checks that each definition has exactly one `::definitions{name="…"}` in the file of its `page`. A directive that matches nothing, names an unknown attribute, or an unregistered name renders a visible `[name: …]` paragraph and logs a warning rather than disappearing. Because `remark-directive` is on for every rulebook file, a line starting with `::` or `:::` in any `.md` is parsed as a directive; `: text` (colon-space) is not.
